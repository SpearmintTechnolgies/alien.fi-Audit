...
- **Path Traversal Protection**: The database service ([db.service.ts](file:///d:/Yash%20Coding2/new/Alien/shared/database/db.service.ts)) validates all read/write file lookup identifiers against an alphanumeric-and-dash regex whitelist (`/^[a-zA-Z0-9-]{10,50}$/`). Directory traversal characters (`..`, `/`, `\`) are immediately rejected.
...
*Developed by & for [Alien](https://alien.fi)*
