...
export function castToFormState(data: Record<string, unknown>): FormState {
  ...
  const casted: FormState = {
    ...INITIAL_FORM_STATE,
    ai_dependence:      (answersData.ai_dependence || INITIAL_FORM_STATE.ai_dependence) as FormState["ai_dependence"],
    monthly_spend_band: (answersData.monthly_spend_band || INITIAL_FORM_STATE.monthly_spend_band) as FormState["monthly_spend_band"],
    spend_visibility:   (answersData.spend_visibility || INITIAL_FORM_STATE.spend_visibility) as FormState["spend_visibility"],
    unit_economics:     (answersData.unit_economics || INITIAL_FORM_STATE.unit_economics) as FormState["unit_economics"],
    main_pain:          (answersData.main_pain || INITIAL_FORM_STATE.main_pain) as FormState["main_pain"],
    leakage_pattern:    (answersData.leakage_pattern || INITIAL_FORM_STATE.leakage_pattern) as FormState["leakage_pattern"],
    ...
    ai_providers:       (aiStack.providers || answersData.ai_providers || INITIAL_FORM_STATE.ai_providers) as string[],
    ai_models:          (aiStack.models || answersData.ai_models || INITIAL_FORM_STATE.ai_models) as string,
    ai_infrastructure:  (aiStack.infrastructure || answersData.ai_infrastructure || INITIAL_FORM_STATE.ai_infrastructure) as string[],
    ai_other:           (aiStack.other || answersData.ai_other || INITIAL_FORM_STATE.ai_other) as string[],
    technical_notes:    (techCtx.technicalNotes || (isNested ? data.technicalNotes : null) || answersData.technical_notes || INITIAL_FORM_STATE.technical_notes) as string,
    documents:          (techCtx.documents || (isNested ? data.documents : null) || answersData.documents || INITIAL_FORM_STATE.documents) as any[],

    architecture_files: (isNested ? (data.architectureFiles || INITIAL_FORM_STATE.architecture_files) : (answersData.architecture_files || INITIAL_FORM_STATE.architecture_files)) as any[],
    cost_files:         (isNested ? (data.costEvidenceFiles || INITIAL_FORM_STATE.cost_files) : (answersData.cost_files || INITIAL_FORM_STATE.cost_files)) as any[],
    usage_metrics: {
